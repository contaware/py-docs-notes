...
In Python all variables are objects, and the variable names are **labels** assigned to objects. Variable names are composed of alphanumeric characters or underscores and are not permitted to commence with a number. Assigning variables is just like giving a new nickname, in `foo = bar = baz = 3` the variable names are just labels for the same object.
